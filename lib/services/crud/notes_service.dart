...
import 'package:mynotes/utilities/extentions/list/filter.dart';
...
class NotesService {
  ...
  Future<DatabaseNote> updateNote(
    ...
    if (updateCount == 0) {
      ...
      //update cache and return updated note
  ...
  Future<int> deleteAllNotes() async {
    await _ensureDBIsOpen();
    final db = _getDatabaseOrThrow();
    final numOfDeletedNotes = await db.delete(noteTable);
    _notes = [];
    _notesStreamController.add(_notes);
  ...
  Future<DatabaseUser> createUser({required String email}) async {
    ...
    final results = await db.query(
      ...
      limit: 1,
      where: 'email = ?',
      whereArgs: [formattedEmail],
    );
    if (results.isNotEmpty) {
      throw UserAlreadyExists();
    }
    final userId = await db.insert(userTable, {email: formattedEmail});
    return DatabaseUser(id: userId, email: formattedEmail);
  }
